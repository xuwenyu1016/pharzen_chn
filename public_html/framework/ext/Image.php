...
class Image {
	//生成图像验证码
    static public function buildImageVerify($width=48,$height=22,$verifyName='verify') {
		if( !isset($_SESSION) ) {
		...
		/*for($i=0;$i<$length;$i++) {
			imagestring($im,5,$i*10+20,mt_rand(1,8)+10,$randval{$i}, $stringColor);
		}*/
		ImageTTFText($im, 20, 0, 4, 25, $stringColor, "/public/common/fonts/tt0102m.ttf", $code);
		self::output($im,'png');
    }
